...
public class DataByAssetChartTab extends LayoutContainer implements MqttConsoleCallback
{
    ...
    public DataByAssetChartTab(DataByAssetView dataView, GwtSession currentSession)
    {
        m_dataView = dataView;
    ...
    public void refresh(boolean removeAllSeries)
    {
        ...
        if (m_chart != null && m_asset != null) {

            if (m_toggleLive.isPressed()) {
                GoogleAnalytics.trackPageview(GoogleAnalytics.GA_DATA_BYASSET_CHART_LIVE_ON);
            }
            else {
                GoogleAnalytics.trackPageview(GoogleAnalytics.GA_DATA_BYASSET_CHART_LIVE_OFF);
            }

            if (removeAllSeries) {
                String asset = m_asset.getAsset();
                m_chart.setTitle(new ChartTitle().setText(asset), null);
            ...
            gwtDataService.findMessagesByAsset(m_currentSession.getSelectedAccount().getName(),
                                               m_asset,
                                               m_metrics,
                                               m_startDate,
                                               m_endDate,
    ...
    public void subscribe()
    {
        ...
        topicQuery.append(m_currentSession.getSelectedAccount().getName())
                  .append("/")
                  .append(m_asset.getUnescapedAsset())
                  .append("/#");
        ...
        // FIXME - if currently connected, we should use the url of the broker that the asset is connected to
        ...
        for (String seriesName : m_series.keySet()) {
            ...
            if (values.keySet().contains(seriesName)) {
                ...
                if (value != null) {
                    ...
                    else if (series.getPoints().length < m_limit &&
                ...
                if (series.getPoints().length < m_limit) {
